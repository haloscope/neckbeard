...
The first ledger written under the duty it introduces. Rows are appended
as gates close; `status` becomes `closed` at Gate 5.
...
| the repo, for an existing home for the LLM judge's rubric | `WORKFLOW.md` already carries process rituals and is already copied by adopters | reused: the rubric becomes a section there rather than a third document | pending |
| `check_harvest.py` and `check_locked.py`, for the positive-control pattern | both carry `--selftest` with deliberate breaks recorded in their design docs | reused: `judge.py` follows the same shape rather than inventing a test harness | pending |
